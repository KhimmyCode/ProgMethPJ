...
public class Werebear extends Enemies implements Attackable {
	...
	public Werebear() {
		...
		for (int i = 0; i < 6; i++) {
			werebearFrames[i] = new Image(
					ClassLoader.getSystemResource("werebear/werebear-walk/werebear-walk" + i + ".png").toString());

		}
		for (int i = 0; i < 12; i++) {
			werebearAttackingFrames[i] = new Image(
					ClassLoader.getSystemResource("werebear/werebear-attack/werebear-attack" + i + ".png").toString());
